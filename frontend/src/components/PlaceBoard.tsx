...
import { QUADRANTS } from "../deployed_objects.json";
...
export default ({ color }: PlaceBoardProps) => {
  const quadrant_responses = QUADRANTS.map((address) =>
    useObjectSync(address, 3)
  );
  ...
  const quadrants = (quadrant_responses as SuiObjectData[]).map((response) => {
    if (response.content?.dataType == "moveObject") {
      return [(response.content.fields as any)["board"], response.digest] as [
        number[][],
        string
      ];
    }
